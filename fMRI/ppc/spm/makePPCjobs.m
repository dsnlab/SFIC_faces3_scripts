...
for subCount = 1:96;
    ...
    for tCount = 1:3
        ...
        fn = ['/Volumes/psych-cog/dsnlab/SFIC/nonbids_data/fMRI/subjects/',subID,'/ses-wave',(num2str(tCount)),'/facesOB'];
    
        if exist(fn, 'dir')
            
            subReoParams{tCount} = reoCell{tCount}(subCount,:);
            matlabbatch{1}.cfg_basicio.file_dir.file_ops.file_fplist.dir = {[projectDir,'/subjects/',subID,'/ses-wave',(num2str(tCount)),'/facesOB']};
            matlabbatch{1}.cfg_basicio.file_dir.file_ops.file_fplist.filter = ['^b_facesOB*'];
            matlabbatch{1}.cfg_basicio.file_dir.file_ops.file_fplist.rec = 'FPList';
            matlabbatch{2}.spm.spatial.realign.estwrite.data{1}(1) = cfg_dep('File Selector (Batch Mode): Selected Files (^b_facesOB*)', substruct('.','val', '{}',{1}, '.','val', '{}',{1}, '.','val', '{}',{1}, '.','val', '{}',{1}), substruct('.','files'));
            ...
            matlabbatch{3}.spm.spatial.coreg.estimate.ref = {[projectDir,'/subjects/',subID,'/ses-wave',(num2str(tCount)),'/structurals/b_hires.nii']};
            ...
            matlabbatch{5}.spm.util.reorient.srcfiles(1) = {[projectDir,'/subjects/',subID,'/ses-wave',(num2str(tCount)),'/structurals/b_hires.nii']};
